$(document).ready(function () {
  ...
  function getFormData (e) {
    ...
    if (!formData.flatType || !formData.postalCode || !formData.price ||
      !formData.area) {
      $('.wrong-data').show();
    } else {
      $('.wrong-data').hide();
  ...
  function setMinMaxAndAverageValues (parsedResponse) {
    $('.price-bar .min-price span')
      .attr('data-min', parsedResponse.minPrice)
      .text(parsedResponse.minPrice.toFixed(2).replace('.', ',') + ' zł');
    $('.price-bar .max-price span')
      .attr('data-max', parsedResponse.maxPrice)
      .text(parsedResponse.maxPrice.toFixed(2).replace('.', ',') + ' zł');
    $('.price-bar .average')
      .attr('data-avg', parsedResponse.avgPrice)
      .text(parsedResponse.avgPrice.toFixed(2).replace('.', ','));
  }
  ...
  function compareRent (e, formData) {
    ...
    if (price > average) {
      ...
      $('.big-paragraph')
        .html('Twój czynsz wypada drogo na tle innych. Dowiedz się dlaczego');
      ...
      $('.big-paragraph')
        .html(
          'Twój czynsz wypada dobrze na tle innych. Możesz nadal skorzystać oczywiście z naszej szczegółowej porównywarki.');
      $('.compared .expensive').show();
    } else {
  ...
  function addUserBill () {
    let val = $(this).val();
    $(this)
      .siblings('.small-price-bar')
      .find('.your-result')
      .attr('data-result', val);
  ...
  function analyseRent (e) {
    ...
    calculateYearToMonthCost.recalculate = function () {
      ...
      $('.input-year-cost .primary--field').each(function () {
        ...
        $(this)
          .parent()
          .find('.small-price-bar')
          .find('.your-result')
          .attr('data-result', calculation.toFixed(2).replace('.', ','));
        calculateSmallChartsBarsLengths();
      });
    };
    ...
    calculateEarnings.$elAdministratorEarnings = $(
      '#bill__oplata_administracja');
    ...
    calculateEarnings.changeAdministratorEarnings = function (whatShouldBeCounted = 'percentage') {
      if (whatShouldBeCounted == 'percentage') {
        let calculation =
          ...
          parseFloat(
            calculateEarnings.$elTotalSpace.val().replace(',', '.') / 100);

        calculateEarnings.$elPercentageOfHouse.val(
      ...
      if (whatShouldBeCounted == 'totalSpace') {
        let calculation =
          parseFloat(calculateEarnings.$elHouseArea.val().replace(',', '.')) /
          (parseFloat(
            calculateEarnings.$elPercentageOfHouse.val().replace(',', '.')) /
            100);

        calculateEarnings.$elTotalSpace.val(
          calculation.toFixed(2).replace('.', ','));
      }
  ...
  function setPriceParameterValues (item) {
    let smallContainer = $(
      '#price-analysis .input-wrapper[data-billId="' + item.slug + '"]');
    ...
    if (item.min_price === item.max_price) {
      ...
      smallContainer.find('.small-price-bar')
        .find('.min-price span')
        .attr('data-min', item.min_price.toFixed(2).replace('.', ','));
      smallContainer.find('.small-price-bar')
        .find('.max-price span')
        .attr('data-max', item.max_price.toFixed(2).replace('.', ','));
      smallContainer.find('.small-price-bar')
        .find('.average')
        .attr('data-avg', item.avg_price.toFixed(2).replace('.', ','));
    }
  }
  ...
  function calculateSmallChartsBarsLengths () {
    ...
    charts.each(function () {
      ...
      let userBarLength = ((userPrice.replace(',', '.') - minPrice) /
  ...
  function isDataTyped () {
    ...
    inputWrappers.each(function () {
      if ($(this).find('input[required]').length &&
        $(this).find('input[required]').val().trim() == '') {
        full = false;
